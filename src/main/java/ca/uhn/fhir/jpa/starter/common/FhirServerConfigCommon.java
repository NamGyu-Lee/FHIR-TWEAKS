...
public class FhirServerConfigCommon {
  ...
  public JpaStorageSettings jpaStorageSettings(AppProperties appProperties) {
    ...
    if(appProperties.getSubscription() != null) {
	  ...
	  // 2023. 10. 17. Search Condition 설정
    jpaStorageSettings.setFilterParameterEnabled(appProperties.getFilter_search_enabled());
	 ...
	 // 2023. 10. 17. ElasticSearch 를 위한 Advance 설정
	  // index의 대한 자동구현, 리소스의 일괄 전송처리
	  // 단 elasticSearch 활용 시 LastN 이 동작하지 않음(Lucene 처리만 진행한듯)
	  // TODO. CONFIG) ElasticSearch 활용 시 ON
	  // jpaStorageSettings.setAdvancedHSearchIndexing(true);
